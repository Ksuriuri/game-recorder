...
private:
    bool ReadKnownValue(const std::string& key, JsonFields& fields)
    {
        ...
        if (key == "movement_speed_scale")
        {
            fields.has_movement_speed_scale = ReadNumber(fields.movement_speed_scale);
            return fields.has_movement_speed_scale;
        }
...
void ApplyMovementSpeedScale(double scale)
{
    const float clamped = static_cast<float>(std::clamp(scale, 0.05, 1.0));
    ...
    if (ped != 0)
    {
        ...
        const float max_blend = clamped < 0.999F
            ...
            : 3.0F;
        (void)invoke<std::uint64_t>(HASH_SET_PED_MAX_MOVE_BLEND_RATIO, ped, max_blend);
    }
}
...
bool ParseControl(const std::string& text, ControlState& control)
{
    ...
    if (fields.has_movement_speed_scale)
    {
        control.movement_speed_scale = std::clamp(fields.movement_speed_scale, 0.05, 1.0);
    }
    return true;
}
...
public:
    ...
    void Tick()
    {
        ...
        ApplyMovementSpeedScale(movement_speed_scale_);
...
private:
    void PollControl()
    {
        ...
        movement_speed_scale_ = control.movement_speed_scale;
    ...
    void Stop(const char* reason)
    {
        ...
        if (movement_speed_scale_ < 0.999)
        {
            ApplyMovementSpeedScale(1.0);
            movement_speed_scale_ = 1.0;
        }
        if (writer_)
        {
    ...
    double movement_speed_scale_ = 1.0;
