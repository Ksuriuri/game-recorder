...
推荐用 `rdr2-camera\install.bat`：把官方 runtime 的 `ScriptHookRDR2.dll`、`dinput8.dll`
放到 `rdr2-camera\vendor\ScriptHookRDR2\` 后，安装器会复制到游戏根目录并构建/部署插件。

手工安装时：

1. 按 ScriptHookRDR2 官方说明安装当前游戏版本适用的 runtime/ASI loader（或使用上面的 vendor）。
